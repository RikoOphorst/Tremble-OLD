...
#define CHECKHR(hr) { HRESULT _hr = hr; if (_hr != S_OK) { _com_error error(_hr); std::cout << error.ErrorMessage() << std::endl; OutputDebugStringA(error.ErrorMessage()); OutputDebugStringA("\n"); DebugBreak(); } }
#define SAFE_RELEASE(ptr) { if (ptr != nullptr) { ptr->Release(); ptr = nullptr; } }
#define GUARANTEE_RELEASE(ptr) { if (ptr != nullptr) { ptr->Release(); ptr = nullptr; } else { const char* string = "Couldn't guarantee release of object"; std::cout << string << std::endl; OutputDebugStringA(string); DebugBreak(); } }
#define SAFE_DELETE(ptr) { if (ptr != nullptr) { delete ptr; ptr = nullptr; } }
#define SAFE_DELETE_FROM_ALLOC(ptr, allocator) { if (ptr != nullptr) { allocator->Deallocate(ptr); } }

#define D3D12_GPU_VIRTUAL_ADDRESS_NULL ((D3D12_GPU_VIRTUAL_ADDRESS)0)
#define D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN ((D3D12_GPU_VIRTUAL_ADDRESS)-1)

#define DESCRIPTOR_ID_UNKNOWN (~(0U))
...
struct CD3DX12_SHADER_BYTECODE : public D3D12_SHADER_BYTECODE
{
	...
	{}
	explicit CD3DX12_SHADER_BYTECODE(const void* _pShaderBytecode, SIZE_T _BytecodeLength)
	{
